...
--*columnas de tablas (cuyo dominio será el que representa el type del objeto)
...
--1) Se define una tabla 'Alumnos' para almacenar objetos de tipo 'Persona'; esta tabla contiene además una columna 'codigo' que se usará como clave primaria
...
--2) Inserción en la tabla de objetos 'Alumnos'. Nótese que puesto que uno de los valores a insertar es de tipo 'Dirección', hay que invocar a su constructor
...
VALUES (2, 'Julia Breña',DIRECCION ('C/Los espartales 25', 'GUADALAJARA', 19004),'18/12/1987');
...
--Para acceder a una variable de instancia del objeto, se hace como si fuera una columna, empleando la notación 'AliasTabla.Campo'. Si esa variable de instancia es a su vez otro objeto, se utiliza la misma notación
--Alumnos que viven en 'Guadalajara'

select * from Alumnos A where A.DIREC.CIUDAD='GUADALAJARA';

--Obtener el código y dirección de todos los alumnos.
...
--Es posible invocar a métodos de objetos (es obligatorio escribir los paréntesis aunque el método no tenga argumentos)

select Nombre, A.Direc.GET_CALLE() from Alumnos A;



--4) Actualización de filas en una tabla de objetos

--Modificamos el nombre de la ciudad a minúscula para aquellos alumnos que vivan en 'Guadalajara'
...
--Borrar a los alumnos de 'guadalajara' (en minúscula)
